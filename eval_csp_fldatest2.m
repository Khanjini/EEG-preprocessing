function accuracy = eval_csp_fldatest2( train_x1, train_x2, n_filter)
...
for it=1:size(sequence,1)
    ...
    % class1, class2 CSP
    cov1=(tmp_train_x1*tmp_train_x1')/trace(tmp_train_x1*tmp_train_x1');
    cov2=(tmp_train_x2*tmp_train_x2')/trace(tmp_train_x2*tmp_train_x2');
    
%     [W D]= CSP_unit_filter(cov1, cov2)
    ...
    for i=1:n_train_trials(1) 
        ...
        x1_cov = (x1_train*x1_train');
        x2_cov = (x2_train*x2_train');
        for j=1:length(csp_pat)
            temp(j)= log(W(:,csp_pat(:,j))'*x1_cov*W(:,csp_pat(:,j)));
            temp1(j)= log(W(:,csp_pat(:,j))'*x2_cov*W(:,csp_pat(:,j)));
        end
    ...
    for i=1:n_test_trials(1) 
        ...
        x1_cov = (x1_test*x1_test');
        x2_cov = (x2_test*x2_test');
        for j=1:length(csp_pat)
            temp(j)= log(W(:,csp_pat(:,j))'*x1_cov*W(:,csp_pat(:,j)));
            temp1(j)= log(W(:,csp_pat(:,j))'*x2_cov*W(:,csp_pat(:,j)));
        end
    ...
    tmp_acc = [tmp_acc; it, acc_test.accuracy, acc_train.accuracy];
...
tmp = [max(tmp_acc)', mean(tmp_acc)', min(tmp_acc)', std(tmp_acc)'];
accuracy = tmp(2:3,:);
